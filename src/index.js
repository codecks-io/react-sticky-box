...
export default class StickyBox extends React.Component {
  ...
  setWidth() {
    if (this.props.width !== "measure") {
      ...
      const reducePadding =
        compStyle.getPropertyValue("box-sizing") === "content-box"
          ? parseInt(compStyle.getPropertyValue("padding-left"), 10) +
            parseInt(compStyle.getPropertyValue("padding-right"), 10)
          : 0;
  ...
  handleScroll = ({ quickCheck } = {}) => {
    ...
    if (
      ...
      bottom &&
      containerHeight + parentTop - parentPaddingBottom <
      ...
      if (this.mode === "notSet") {
        ...
        if (viewPortHeight >= nodeHeight) {
          ...
          // scroll up and node taller than window
          ...
          if (this.mode === "fixedTop") {
            ...
            if (
              scrollPaneOffsetTopWithScroll + viewPortHeight >=
              nodeHeight + parentTop + this.plainOffset - parentPaddingBottom
            ) {
